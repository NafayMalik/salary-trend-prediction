...
url = 'https://www.alphavantage.co/query?function=REAL_GDP&interval=annual&apikey=JR94ONX7SUG028LJ'
...
for i in range(len(result)):
    ...
    for item in data:
        ...
        if api_year == work_year:
            cursor_obj.execute(f'update data set gdp = {apivalue} where work_year = {api_year};')
            conn.commit()
            print(f'For year "{work_year}" GDP value is "{apivalue}"')
